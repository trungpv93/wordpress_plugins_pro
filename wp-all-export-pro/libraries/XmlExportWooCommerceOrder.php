									...
									if ( $options['cc_type'][$subID] == 'woo_order' and $options['cc_options'][$subID] == 'taxes') {
										...
										switch ($options['cc_value'][$subID]) {
											...
											case '_order_shipping':													
													$shipping_data[$element_name] = pmxe_filter( get_post_meta($record->ID, '_order_shipping', true), $ShippingfieldSnipped);													
												break;
										}																				
									}
								}		
							...
							foreach ($this->order_surcharge as $order_surcharge) {									
								...
								foreach ($options['ids'] as $subID => $subvalue) {
									...
									if ( $options['cc_type'][$subID] == 'woo_order' and $options['cc_options'][$subID] == 'fees') {
										...
										// $element_name = str_replace("Amount (per surcharge)", "Amount", $options['cc_name'][$subID]);
						...
						{																					
							foreach ($data as $n => $item) {																	
								if ( ! empty($item))
								{
									foreach ($item as $item_key => $item_value) 
									{
										$final_key = (strpos($item_key, "#") === false and ! in_array($key, array('taxes', 'shipping', 'coupons', 'surcharge'))) ? $item_key . " #" . ($n + 1) : $item_key;
								...
								if ( ! empty($item)){
									...
									foreach ($item as $item_key => $item_value) {
										...
										$xmlWriter->beginElement($element_name_ns, $element_name, null);
											$xmlWriter->writeData($item_value);
